...
def hat_so3(omega):
    """
    将 3×1 向量 omega = [ωx, ωy, ωz]^T 转换为反对称矩阵 [omega]_x，
    该矩阵满足对于任意 3×1 向量 v，有 [omega]_x v = omega × v

    参数:
      omega: (3,) 或 (3,1) 的 numpy 数组，表示 3D 角速度向量

    返回:
      3x3 的反对称矩阵
    """
    omega = np.squeeze(omega)  # 确保 omega 为一维数组
    return np.array([
        [0,        -omega[2],  omega[1]],
        [omega[2],  0,        -omega[0]],
        [-omega[1], omega[0],  0       ]
